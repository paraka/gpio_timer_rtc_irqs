...
static int intc_init_function(u16 DeviceId,  XTmrCtr *TmrInstancePtr, XGpio *GpioInstancePtr);
...
static int intc_init_function(u16 DeviceId, XTmrCtr *TmrInstancePtr, XGpio *GpioInstancePtr)
{
	...
	status = XScuGic_Connect(&INTCInst,
							 ...
							 (Xil_ExceptionHandler)timer_irq_handler,
							 (void *)TmrInstancePtr);
	...
	/* Enable timer in the interrupt controller */
...
int main (void)
{
	...
	XTmrCtr_SetHandler(&TMRInst, (XTmrCtr_Handler)timer_irq_handler, &TMRInst);
	XTmrCtr_SetOptions(&TMRInst, 0, XTC_INT_MODE_OPTION | XTC_AUTO_RELOAD_OPTION);
	XTmrCtr_SetResetValue(&TMRInst, 0, TIMER_LOAD_REGISTER);
	XTmrCtr_Start(&TMRInst,0);

	/* Initialize interrupt controller */
	status = intc_init_function(INTC_DEVICE_ID, &TMRInst, &BTNInst);
